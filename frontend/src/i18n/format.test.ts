...
describe('a day inside a sentence', () => {
  ...
  it('bends the month in Serbian and leaves every other language alone', () => {
    /* The rule is Serbian grammar and it is applied to Serbian and to nothing else.
       Measured 26.09.2026, on the run that wired the English dictionary: applied to
       whatever `formatDate` returns, this answered „Octobera 1, 2026" for English, a
       word in no language at all. The same fault had been found once before from the
       other side, when `/en` drew the Serbian dictionary and the date was asked with
       the address's language (review, 05.09.2026).

       English needs no rule here at all: „it opens on October 1, 2026" is already the
       form a verb governs. */
    expect(formatDayInSentence('2026-10-01', 'sr')).toBe('1. oktobra 2026.')
    expect(formatDayInSentence('2026-10-01', 'en')).toBe('October 1, 2026')
    /* Every month, so a rule that bent only the four ending in „bar" cannot pass by
       leaving the other eight to English. */
    expect(formatDayInSentence('2026-03-01', 'en')).toBe('March 1, 2026')
  })

  it('reads the language of the words, not the language of the address', () => {
    /* One lookup for both halves. The month comes out of the formatter and the grammar
       is put on it afterwards, so the two have to be told the same language: a formatter
       asked for the address's language beside a rule asked for the sentence's is how
       „Octobera" was made in the first place.

       This is the case that fails the moment somebody takes `dictionaryLocale` back out
       of it: the one language every leg here reads is the language of the words. */
    expect(formatDayInSentence('2026-10-01', DEFAULT_LOCALE)).toBe(
      formatDayInSentence('2026-10-01', 'sr'),
    )
    expect(formatDayInSentence('2026-10-01', 'en')).not.toBe(
      formatDayInSentence('2026-10-01', 'sr'),
    )
    /* And the day standing on its own follows the address, which is what the screens
       that show a date as a value ask for and what this does not change. */
