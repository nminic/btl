...
/**
 * The answer both forms on an event give when the event has not been run.
 *
 ...
 */
export function NotRunYet({ slug }: { slug: string }) {
  const { locale, t } = useI18n()

  return (
    <div className="member">
      <h1>{t('event.notRunYet')}</h1>
      <p>{t('event.notRunYetWhy')}</p>
