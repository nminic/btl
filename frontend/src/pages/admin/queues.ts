 ...
 *
 * All five have a screen, and every screen asks the same first question: is this
 ...
 *
 * - `sendBack`: approve, or hand the work back with a reason. Three of the five.
 ...
 * So it is `sendBack` like the other four, and the exception is the comment,
...
/**
 * The five, each under its own id, so a screen can be handed the queue it
 ...
 * that has never been written anywhere. This way every one of the five is there
 * because the type says which five there are, and nothing is proved twice.
 *
...
export function canSendBack(
   ...
   *
     Since 15.08.2026 that is four queues rather than one (owner: „Poruka ide sa
 ...
 * says.
 *
 * Four queues out of five, therefore, since 15.08.2026 (owner: „Poruka ide sa
...
/**
 * Everything the five numbers are counted from, and nothing else.
 *
 ...
 * `items` with the other four. The field outlived its reader by one release, and
 ...
 *
 * Over the queues given, which is not always all five: a moderator sees only
