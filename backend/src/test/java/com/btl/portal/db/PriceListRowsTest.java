...
/**
 * The six rows of the price list, and the shape ADL A36 O12 decided for them.
 *
 ...
 * here instead is the decision. Six rows and three kinds of row, four periods,
 * one level, one fee; the fee is the only row with no dinar price; only a period
 * answers whether what it buys is ranked; and the four periods tile the year with
 ...
 * `pricing.ts` and this from PDL P8 and the decisions of 03. and 04.08.2026.
 */
class PriceListRowsTest extends DatabaseTest {
	...
	/**
	 * Six rows, and the amounts each one carries.
	 *
	 ...
	 */
	@Test
	void thePriceListIsTheSixRowsTheOwnerDecided() {
	...
	@Test
	void thereAreThreeKindsOfRowAndOneOfThemIsThePeriods() {
		List<Row> rows = rows();

		assertThat(rows).extracting(Row::kind).containsOnly("period", "level", "fee");
	...
	/**
	 * Only a period answers the ranking question, and the other two leave it empty.
	 *
	 ...
	 * than answering; the fee buys nothing in the rulebook at all. Both public
